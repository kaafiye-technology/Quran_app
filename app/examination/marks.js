...
// const App = () => {
  ...
  // ];
  const App = () => {
    const tableHead = ['Course', 'Total', 'Status'];
    // const tableData = [
...
useEffect(()=>{
  ...
  const tableRows = marks.map(item => [item.course, item.total, item.status]);
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container}>
        <Card containerStyle={styles.card}>
          <Text h3 style={styles.title}>Examination Result</Text>
          <Table borderStyle={{ borderWidth: 1, borderColor: '#C1C0B9' }}>
            <Row data={tableHead} style={styles.head} textStyle={styles.headText} />
            <Rows data={tableRows} textStyle={styles.text} />
            <Row data={['Total', sum,roundedNumber+'%' ]} style={styles.footer} textStyle={styles.footerText} />
          </Table>
        </Card>
      </ScrollView>
    </SafeAreaView>
